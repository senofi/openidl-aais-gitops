  ...
  policy = jsonencode({
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Effect": "Allow",
            "Principal": {
                "AWS": local.org_name == "anal" ? ["${aws_iam_role.openidl_apps_iam_role.arn}", "${var.aws_role_arn}",
#                  "${aws_iam_role.etl_lambda.arn}", "${aws_iam_role.upload.arn}", "${aws_iam_role.etl_lambda.arn}", "${aws_iam_role.upload.arn}"
                  "${aws_iam_role.reporting_lambda[0].arn}"] :  ["${aws_iam_role.openidl_apps_iam_role.arn}", "${var.aws_role_arn}" ]
            },
            "Action": [
                "kms:Encrypt",
                "kms:Decrypt",
